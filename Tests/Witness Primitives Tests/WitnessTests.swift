...
struct WitnessMacroTests {
    ...
    // NOTE: `unimplemented` test removed.
    // Per [API-IMPL-003] totality requirements, `unimplemented` with fatalError
    // is not generated by the primitives macro. Test-aware `unimplemented` is
    // provided by the foundations layer (swift-witnessess).
    ...
    func actionCaseEnumerable() {
        ...
        // Verify init(__unchecked:_:)
        #expect(API.Action.Case(__unchecked: (), 0) == .fetchUser)
        #expect(API.Action.Case(__unchecked: (), 1) == .deleteUser)
    ...
    @Test("Prism composition via dynamicMemberLookup")
    func prismCompositionViaDynamicMemberLookup2() {
        let outer = Outer.inner(.value(42))
    ...
    func enumConformsToPrismAccessible() {
        ...
        func requiresAccessible<T: Optic.Prism.Accessible>(_ type: T.Type) {}
